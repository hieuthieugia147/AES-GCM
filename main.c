...
static void single_encryption(void) {
    ...
    handle_gcm_init( &ctx );
    // 128 bits, not bytes!
    ret = handle_gcm_setkey( &ctx, cipher, key, 128 );
    ret = handle_gcm_crypt_and_tag(&ctx, handle_GCM_ENCRYPT, 64, initial_value, 12, additional, 0, plaintext, buf, 16, tag_buf);
    handle_gcm_free( &ctx );
